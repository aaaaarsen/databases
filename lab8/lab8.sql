...
--task6
CREATE VIEW top_customers AS (SELECT * FROM customers WHERE grade = (SELECT MAX(grade) FROM customers));

GRANT SELECT ON top_customers TO junior_dev;

--task7
CREATE VIEW salesman_count AS (SELECT city, COUNT(salesman_id) AS num_salesmen FROM salesman GROUP BY city);
